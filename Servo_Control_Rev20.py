...
MEDIUM_PRIORITY_UPDATE_INTERVAL = .5  # Reduced Arduino polling (sec)
LOW_PRIORITY_UPDATE_INTERVAL = 1  # Faster display updates (sec)
BATCH_SIZE = 10
                ...
                match event:
                    ...
                    case _ if event.endswith('B3'): # Handle Setpoint Update                                
                        ...
                        if all(x is not None for x in (V_data, A_data, P_data)):
                            ...
                            window[f'S{servo}P_SPT_display'].update(P_data)
                            window.refresh()
                            print(f"Debug 46 - Updated setpoints for S{servo}: V_SPT={V_data}, A_SPT={A_data}, P_SPT={P_data}")  # Add debug print
