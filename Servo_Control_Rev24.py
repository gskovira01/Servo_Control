...
def try_initialize():
    success = initialize_from_arduino(window)
    if not success:
...
# Start initialization in a background thread so the GUI stays responsive
...
while True:
    ...
    # Always check for initialization errors from the background thread and show popup from main thread
    try:
    ...
    # Catch any unexpected errors in the event loop
    # (kept as a safety net, but should not catch queue.Empty for popup)
    #
    # If you want to log or handle other exceptions, do it here
    #
    # except Exception as e:
...
udp_sock.close()
window.close()
# Do not call exit(1) here; let the user close the window manually
