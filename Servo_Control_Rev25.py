...
setpoint_values_1 = {
    ...
    'S1V_SPT': 1000, 'S1A_SPT': 2000, 'S1P_SPT': 0,
    'S2V_SPT': 1000, 'S2A_SPT': 2000, 'S2P_SPT': 0,
    'S3V_SPT': 1000, 'S3A_SPT': 2000, 'S3P_SPT': 0,
    'S4V_SPT': 1000, 'S4A_SPT': 2000, 'S4P_SPT': 0
}
...
def initialize_values(window, message_queue):
...
def initialize_from_arduino(window, send_udp_command, message_queue):
    ...
    if not initialize_values(window, message_queue):
        return False
...
[
    ...
    sg.Button('Started' if GUI_button_states['Start'] else 'Start', key=prefix+'Start',
        ...
        [sg.Text(' ', size=(32, 1)), sg.Text('Velocity', size=(6, 1)), sg.Text('Acceleration', size=(9, 1)), sg.Text('Position', size=(7, 1))],
    ]
    for i in range(1, 5):
        panel += [
            [sg.Text(f'Servo {i}', size=(8, 1)),
             ...
             sg.InputText(setpoint_values[f'S{i}V_SPT'], key=prefix+f'S{i}V_SPT', size=(8, 1), justification='right'),
             sg.InputText(setpoint_values[f'S{i}A_SPT'], key=prefix+f'S{i}A_SPT', size=(8, 1), justification='right'),
             sg.InputText(setpoint_values[f'S{i}P_SPT'], key=prefix+f'S{i}P_SPT', size=(8, 1), justification='right'),
             sg.Button('OK', key=prefix+f'S{i}B3', size=(8, 2))],
            [sg.Text(' ', size=(17, 1)), sg.Text('Current Value', size=(14, 1), justification='right'),
            sg.Text(arduino_values[f'S{i}V'], size=(7, 1), key=prefix+f'S{i}V_display', justification='right'),
...
def try_initialize():
    success1 = initialize_from_arduino(window, send_udp_command1, message_queue)
    success2 = initialize_from_arduino(window, send_udp_command2, message_queue)
    if not (success1 and success2):
        init_error_queue.put("Failed to initialize from ClearCore.\nCheck your hardware and network, then try again.")

threading.Thread(target=try_initialize, daemon=True).start()
        ...
        if event not in last_event_time or (current_time - last_event_time[event] > DEBOUNCE_INTERVAL):
            ...
            match event_key:
                ...
                case _ if event_key.endswith('B3'):
                    servo = int(event_key[1])
                    V_data = validate_input(f'B{board_num}_S{servo}V_SPT', values, 0, 18000)
                    A_data = validate_input(f'B{board_num}_S{servo}A_SPT', values, 0, 18000)
                    P_data = validate_input(f'B{board_num}_S{servo}P_SPT', values, 0, 18000)
                    if all(x is not None for x in (V_data, A_data, P_data)):
