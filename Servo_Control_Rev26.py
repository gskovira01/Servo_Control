    ...
    for i in range(1, 5):
        panel += [
            ...
            [sg.Text(' ', size=(8, 1)),
             ...
             sg.Text('Current Value', size=(10, 1), justification='right'),
            ...
            match event_key:
                ...
                case _ if event_key.endswith('V_SPT_btn'):
                    ...
                    new_value = sg.popup_get_text(
                        f'Enter Velocity Setpoint for Servo {servo}:',
                    ...
                    if new_value and new_value.isdigit():
                        value = int(new_value)
                        if 0 <= value <= 18000:
                            setpoint_values[f'S{servo}V_SPT'] = value
                            window[event].update(str(value))
                case _ if event_key.endswith('A_SPT_btn'):
                    servo = int(event_key[1])
                    current_value = setpoint_values[f'S{servo}A_SPT']
                    new_value = sg.popup_get_text(
                        f'Enter Acceleration Setpoint for Servo {servo}:',
                        title='Acceleration Setpoint',
                        default_text=str(current_value),
                        size=(20, 1)
                    )
                    if new_value and new_value.isdigit():
                ...
                case _ if event_key.endswith('P_SPT_btn'):
                    ...
                    if new_value and new_value.isdigit():
                        value = int(new_value)
                        if 0 <= value <= 18000:
                            setpoint_values[f'S{servo}P_SPT'] = value
                            window[event].update(str(value))
