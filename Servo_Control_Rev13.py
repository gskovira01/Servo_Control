...
def handle_servo_buttons(event, enable_command, disable_command, enabled):
    """Handle servo button events and state changes."""
    match event:
        case 'Mode':  # Mode is either Manual or Auto
            if enabled:
        ...
        case 'S1B1' | 'S2B1' | 'S3B1' | 'S4B1':  # B1 is either ENABLE or DISABLE
            if enabled:
                serialInst.write(f"CMD:{disable_command}\n".encode('utf-8'))
                ...
                GUI_button_states[event] = True
        case 'S1B2' | 'S2B2' | 'S3B2' | 'S4B2':  # B2 is either Run or Stop
            if enabled:
                serialInst.write(f"CMD:{disable_command}\n".encode('utf-8'))
...
layout = [
    ...
    [sg.Text(' ', size=(8, 1)), sg.Button('Manual' if GUI_button_states['Mode'] else 'Auto',key='Mode', button_color=('white', 'red') if GUI_button_states['Mode'] else ('white', 'green')),
        ...
        if event and isinstance(event, str):
            ...
            match event:
                case 'Mode':
                    ...
                    CNT_button_states[event] = handle_servo_buttons(event,"Mode AUTO","Mode MANUAL",CNT_button_states[event])
                    window.refresh()
                ...
                case 'S1B1' | 'S2B1' | 'S3B1' | 'S4B1':
                    ...
                    GUI_button_states[event] = not GUI_button_states[event]
                    if event == 'S1B1':
                        S1B1 = handle_servo_buttons(event, "S1B1 ENABLE", "S1B1 DISABLE", S1B1)
                    elif event == 'S2B1':
                        S2B1 = handle_servo_buttons(event, "S2B1 ENABLE", "S2B1 DISABLE", S2B1)
                    elif event == 'S3B1':
                        S3B1 = handle_servo_buttons(event, "S3B1 ENABLE", "S3B1 DISABLE", S3B1)
                    elif event == 'S4B1':
                        S4B1 = handle_servo_buttons(event, "S4B1 ENABLE", "S4B1 DISABLE", S4B1)
                case 'S1B2' | 'S2B2' | 'S3B2' | 'S4B2':
                    servo = int(event[1])
                    GUI_button_states[event] = not GUI_button_states[event]
                    if event == 'S1B2':
                        S1B2 = handle_servo_buttons(event, "S1B2 START", "S1B2 STOP", S1B2)
                    elif event == 'S2B2':
                        S2B2 = handle_servo_buttons(event, "S2B2 START", "S2B2 STOP", S2B2)
                    elif event == 'S3B2':
                        S3B2 = handle_servo_buttons(event, "S3B2 START", "S3B2 STOP", S3B2)
                    elif event == 'S4B2':
                        S4B2 = handle_servo_buttons(event, "S4B2 START", "S4B2 STOP", S4B2)
