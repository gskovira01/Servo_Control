...
VERSION: Rev 30 (Enhanced Network Handling)
DATE: October 2025
...
arduino_values_2 = arduino_values_1.copy()              # Independent state for Board 2
...
GUI_button_states_2 = GUI_button_states_1.copy()       # Independent button states for Board 2
...
def process_values_response(message, window, arduino_values, prefix):
    ...
    if len(parts) >= 12:
        ...
        # Update Position displays (position only - velocity and acceleration hidden)
        window[prefix+'S1P_display'].update(arduino_values['S1P'])
        # Update Positions 2-4 displays (position only - simplified interface)
...
def build_board_panel(board_num, arduino_values, setpoint_values, GUI_button_states):
    ...
    Interface Layout:
    - Servo 1: Full controls (Velocity, Acceleration, Position)
    - Servos 2-4: Position-only controls (Velocity/Acceleration hidden)
    
    This provides a cleaner interface while maintaining full control capability
    for the primary servo and simplified control for auxiliary servos.
    """
    ...
    for i in range(1, 5):
        if i == 1:
            # Servo 1: Show all controls (Velocity, Acceleration, Position) - Hide Enable/Start buttons
            panel += [
                ...
                [sg.Button('Clear Value', key=prefix+f'S{i}B4', size=(16, 1), button_color=('black', 'orange'), font=CLEAR_BUTTON_FONT),
                 ...
                 sg.Text('..............................', size=(31, 1), font=GLOBAL_FONT),  # Adjusted spacing (same as Servo 1)
                 sg.Text(arduino_values[f'S{i}P'], size=(6, 1), key=prefix+f'S{i}P_display', justification='center', font=GLOBAL_FONT)]
            ]
        else:
            # Servos 2, 3, 4: Hide velocity, acceleration, and enable/start buttons - show only position
            # RASPBERRY PI FIX: Add extra spacing to align Position and OK buttons with Servo 1
            panel += [
                [sg.Text(f'Position {i}', size=(13, 1), justification='left', font=POSITION_LABEL_FONT),
                 sg.Text('', size=(17, 1), font=GLOBAL_FONT),  # Hidden Enable/Disable button (same as Servo 1)
                 sg.Text('', size=(8, 1), font=GLOBAL_FONT),   # Hidden Start/Stop button (same as Servo 1)
                 sg.Text('', size=(8, 1), font=GLOBAL_FONT),   # Hidden velocity control (same size as velocity button)
                 sg.Text('', size=(8, 1), font=GLOBAL_FONT),   # Hidden acceleration control (same size as accel button)
                 sg.Text('', size=(8, 1), font=GLOBAL_FONT),   # Extra spacing to push Position button right for Pi alignment
                 sg.Button(f'{setpoint_values[f"S{i}P_SPT"]}', key=prefix+f'S{i}P_SPT_btn', size=(8, 1), button_color=('black', 'lightblue'), font=GLOBAL_FONT),
                 sg.Button('OK', key=prefix+f'S{i}B3', size=(8, 2), font=GLOBAL_FONT)],
                [sg.Button('Clear Value', key=prefix+f'S{i}B4', size=(16, 1), button_color=('black', 'orange'), font=CLEAR_BUTTON_FONT),
                 sg.Text('', size=(5, 1), font=GLOBAL_FONT),  # 5-space spacing after Clear Value button (same as Servo 1)
                 sg.Text('Current Position:', size=(16, 1), justification='left', font=GLOBAL_FONT),
                 sg.Text('..............................', size=(31, 1), font=GLOBAL_FONT),  # Adjusted spacing (same as Servo 1)
                 sg.Text(arduino_values[f'S{i}P'], size=(6, 1), key=prefix+f'S{i}P_display', justification='center', font=GLOBAL_FONT)]
            ]
...
# Build the main layout with optional shutdown button - Tab 2 hidden
main_layout = [
    [sg.TabGroup(
        [[
            sg.Tab('Board 1', build_board_panel(1, arduino_values_1, setpoint_values_1, GUI_button_states_1), key='TAB1')
            # sg.Tab('Board 2', build_board_panel(2, arduino_values_2, setpoint_values_2, GUI_button_states_2), key='TAB2')  # Hidden
        ]],
        key='TABGROUP',
