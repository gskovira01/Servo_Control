...
GUI_UPDATE_INTERVAL = 0.5  # Faster display updates (sec)
BATCH_SIZE = 50
            ...
            try:
                if self.serial_port.in_waiting:
                    ...
                    if line:
                        ...
                        if DEBUG:
                            print(f"Debug00 - Serial message received: {line}")
                else:
                    time.sleep(0.01)  # Prevent CPU hogging
            except Exception as e:
                print(f"Debug01 - Serial error: {e}")
                self.running = False
...
def handle_servo_buttons(event, enable_command, disable_command, enabled, window):
    ...
    print(f"Debug123 - Handling event: {event}, enabled: {enabled}")  # Add debug print
    
    if event == 'Mode':  # Mode is either Manual or Auto
        if enabled:
            print(f"Debugs1 - Sending disable command for Mode: MANUAL")  # Add debug print
            serialInst.write(f"CMD:Mode MANUAL\n".encode('utf-8'))
            window[event].update(text='Auto', button_color=('white', 'green'))
            GUI_button_states[event] = False
        else:
            print(f"Debugs2 - Sending enable command for Mode: AUTO")  # Add debug print
        ...
        if enabled:
            print(f"Debugs3 - Sending disable command for REPEAT: DISABLE")  # Add debug print
            serialInst.write(f"CMD:REPEAT ENABLE\n".encode('utf-8'))
            window[event].update(text='Enable Repeat', button_color=('white', 'green'))
            GUI_button_states[event] = False
        else:
            print(f"Debugs4 - Sending enable command for REPEAT: ENABLE")  # Add debug print
        ...
        if enabled:
            print(f"Debugs5 - Sending disable command for START: DISABLE")  # Add debug print
            serialInst.write(f"CMD:START DISABLE\n".encode('utf-8'))
            window[event].update(text='Disable Start', button_color=('white', 'green'))
            GUI_button_states[event] = False
        else:
            print(f"Debugs6 - Sending enable command for START: ENABLE")  # Add debug print
        ...
        if enabled:
            print(f"Debugs7 - Sending disable command for {event}: DISABLE")  # Add debug print
            serialInst.write(f"CMD:{event} DISABLE\n".encode('utf-8'))
            window[event].update(text='ENABLE', button_color=('white', 'green'))
            GUI_button_states[event] = False
        else:
            print(f"Debugs8 - Sending enable command for {event}: ENABLE")  # Add debug print
        ...
        if enabled:
            print(f"Debugs9 - Sending disable command for {event}: STOP")  # Add debug print
            serialInst.write(f"CMD:{event} STOP\n".encode('utf-8'))
            window[event].update(text='RUN', button_color=('white', 'green'))
            GUI_button_states[event] = False
        else:
            print(f"Debugs10 - Sending enable command for {event}: START")  # Add debug print
        ...
        if all(x is not None for x in (V_data, A_data, P_data)):
            ...
            print(f"Debugs11 - Sending command: {cmd.strip()}")  # Add debug print
            ...
            window.refresh()
            print(f"Debugs12 - Updated setpoints for S{servo}: V_SPT={V_data}, A_SPT={A_data}, P_SPT={P_data}")  # Add debug print
            ...
            if event not in last_event_time or (current_time - last_event_time[event] > DEBOUNCE_INTERVAL):
                ...
                match event:
                    case 'Mode':
                        ...
                        window.refresh()
                        print(f"DebugW1 - Sending command: {'Manual Mode' if GUI_button_states[event] else 'Auto'}")
                    case 'REPEAT':
                        GUI_button_states[event] = not GUI_button_states[event]
                        CNT_button_states[event] = handle_servo_buttons(event, "REPEAT ENABLED", "REPEAT DISABLED", CNT_button_states[event], window)
                        window[event].update(text='Auto' if GUI_button_states[event] else 'Manual', button_color=('white', 'green') if GUI_button_states[event] else ('black', 'yellow'))
                        window.refresh()
                        print(f"DebugW2 - Sending command: {'REPEAT ENABLED' if GUI_button_states[event] else 'REPEAT DISABLED'}")
                    case 'START':
                        GUI_button_states[event] = not GUI_button_states[event]
                        ...
                        window.refresh()
                        print(f"DebugW3 - Sending command: {'STARTED' if GUI_button_states[event] else 'NOT STARTED'}")
                    ...
                    case _ if event.endswith('B3'): # Handle Setpoint Update                                
                        ...
                        if all(x is not None for x in (V_data, A_data, P_data)):
                            cmd = f"CMD:S{servo}_Parameters:{V_data},{A_data},{P_data}\n"
                            print(f"DebugW5 - Sending command: {cmd.strip()}")  # Add debug print
                            serialInst.write(cmd.encode('utf-8'))
                            serialInst.flush()
                            # Update current_values and corresponding sg.Text elements
                            current_values[f'S{servo}V_SPT'] = V_data
                            ...
                            window[f'S{servo}P_SPT_display'].update(P_data)
                            window.refresh()
                            print(f"DebugW6 - Updated setpoints for S{servo}: V_SPT={V_data}, A_SPT={A_data}, P_SPT={P_data}")  # Add debug print
        ...
        if current_time - last_gui_update > GUI_UPDATE_INTERVAL:
            ...
            try:
                for _ in range(BATCH_SIZE):
                    ...
                    if DEBUG:
                        print(f"DebugP1 - Processing message: {message}")
